...
class Goal:
    ...
    @staticmethod
    def create(user_id, title, description=None, target_date=None, status='in_progress'):
        """Create a new goal."""
        db = get_db()
        db.execute(
            '''INSERT INTO goals 
    ...
    @staticmethod
    def update(goal_id, title, description=None, target_date=None, status=None):
        """Update a goal."""
        db = get_db()
        ...
        target_date = target_date if target_date is not None else goal['target_date']
